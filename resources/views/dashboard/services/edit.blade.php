                        ...
                        <form action="{{ route('services.update', $service->id) }}" method="POST" id="service"
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="text" name="name" id="product-name" class="form-control"
                                    placeholder="e.g : Apple iMac" value="{{ $service->name }}">
                                @error('name')
                            ...
                            <div class="fallback">
                                <input type="file" name="image" type="file"
                                    src=" {{ asset('assets/img/services/icons/' . $service->image) }}" />
                                @error('image')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        @foreach ($icons as $icon)
                            <div id="show_item">
                                <div class="row">
                                    ...
                                    <input type="text" id="icon_name" value="{{ $icon->name }}" name="icon_name[]"
                                        class="form-control">
                                    @error('icon_name')
                                        <div class="alert alert-danger">{{ $message }}</div>
                                    @enderror
                                    ...
                                    <input type="file" id="icon_image" name="icon_image[]"
                                        value=" {{ $service->image }}" class="form-control">
                                    @error('icon_image')
                                        <div class="alert alert-danger">{{ $message }}</div>
                                    @enderror
